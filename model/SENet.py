...
class BottleneckBlock(nn.Module):
	def __init__(self, in_channels, out_channels, r, drop_rate):
		...
		self.left = nn.Sequential(
			nn.Conv2d(in_channels=in_channels, out_channels=out_channels, kernel_size=1,stride=drop_rate, padding=0, bias=False),
			nn.BatchNorm2d(out_channels),
			nn.ReLU(inplace=True),
			nn.Conv2d(in_channels=out_channels, out_channels=out_channels, kernel_size=3, padding=1, bias=False),
			nn.BatchNorm2d(out_channels),
...
class SENet(nn.Module):
	...
	[N, C_in, H, W] -> [N, C_out, H / (drop_rate ^ blocks), W / (drop_rate ^ blocks)]
	'''
...
class SENet_decoder(nn.Module):
	...
	[N, C_in, H, W] -> [N, C_out * (drop_rate)^(blocks-1), H / (drop_rate)^(blocks-1), W / (drop_rate)^(blocks-1)]
	'''
	...
	def __init__(self, in_channels, out_channels, blocks, block_type="BottleneckBlock", r=8, drop_rate=2):
		...
		for _ in range(blocks - 1):
			layer1 = eval(block_type)(out_channels, out_channels, r, drop_rate=1)
			layers.append(layer1)
			layer2 = eval(block_type)(out_channels, out_channels * drop_rate, r, drop_rate=drop_rate)
