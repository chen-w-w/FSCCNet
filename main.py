...
def main():
    ...
    parent_parser = argparse.ArgumentParser(description='Training of FSCCNet nets')
    ...
    new_run_parser.add_argument('--enable-fp16', dest='enable_fp16', action='store_true', # dest='enable_fp16'表示要用args.enable_fp16才能访问
    ...
    # Restart training
        ...
        net_config = HiDDenConfiguration(H=args.size, W=args.size,
                                            ...
                                            encoder_blocks=4, encoder_channels=24,
                                            decoder_blocks=7, decoder_channels=24,
                                            use_discriminator=True,
                                            use_vgg=False,
                                            discriminator_blocks=3, discriminator_channels=64,
                                            decoder_loss=1,
                                            encoder_loss=0.7,
                                            mse_weight=0.6,ssim_weight=0.3,
                                            decode_weight=4,
                                            adversarial_loss=1e-1,
    ...
    if (args.command == 'new' and args.tensorboard) or \
        ...
        # tb_logger = SummaryWriter(log_dir=os.path.join(this_run_folder, 'tb-logs'))
    ...
    model = FSCCNet(net_config, device, noiser, tb_logger)
