...
from model.FSCCNet import FSCCNet
...
def save_checkpoint(model: FSCCNet, experiment_name: str, epoch: int, checkpoint_folder: str):
...
def model_from_checkpoint(FSCCNet, checkpoint):
    """ Restores the net object from a checkpoint object """
    FSCCNet.encoder_decoder.load_state_dict(checkpoint['enc-dec-model'])
    FSCCNet.optimizer_enc_dec.load_state_dict(checkpoint['enc-dec-optim'])
    FSCCNet.discriminator.load_state_dict(checkpoint['discrim-model'])
    FSCCNet.optimizer_discrim.load_state_dict(checkpoint['discrim-optim'])
...
def get_data_loaders(network_config: HiDDenConfiguration, train_options: TrainingOptions, train_size=10000,val_size=3000,seed=42):
    ...
    data_transforms = {
        'train': transforms.Compose([
            ...
            transforms.ToTensor(),
            transforms.Normalize([0.5, 0.5, 0.5], [0.5, 0.5, 0.5])  # 范围从[0,1] -> [-1,1]
        ]),
    ...
    full_train_images = datasets.ImageFolder(train_options.train_folder, data_transforms['train'])

    # 选择子集
    # subset_size = int(len(full_train_images) * subset_ratio)
    remaining_size = len(full_train_images) - train_size
    train_subset, _ = random_split(full_train_images, [train_size, remaining_size])
    train_loader = torch.utils.data.DataLoader(train_subset, batch_size=train_options.batch_size, shuffle=True,
                                               num_workers=4)

    full_validation_images = datasets.ImageFolder(train_options.validation_folder, data_transforms['val'])
    remaining_size = len(full_validation_images) - val_size
    val_subset, _ = random_split(full_validation_images, [val_size, remaining_size])
    validation_loader = torch.utils.data.DataLoader(val_subset, batch_size=train_options.batch_size,
