...
class Dropout(nn.Module):
    ...
    def forward(self, noised_and_cover):
        ...
        mask_percent = np.random.uniform(self.keep_min, self.keep_max)# random number in [min,max] 丢弃的面积比率

        mask = np.random.choice([0.0, 1.0], noised_image.shape[2:], p=[mask_percent, 1 - mask_percent])
