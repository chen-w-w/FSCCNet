...
def train(model: FSCCNet,
    ...
    for epoch in range(train_options.start_epoch, train_options.number_of_epochs + 1):
        ...
        logging.info('Epoch {} training duration {:.2f} sec'.format(epoch, train_duration)) # 单个epoch训练时间
        logging.info('-' * 40)
        ...
        for image, _ in val_data:
            ...
            if first_iteration:
                ...
                # utils.save_images(image.cpu()[:images_to_save, :, :, :],    # 原图
                #                   encoded_images[:images_to_save, :, :, :].cpu(),   # 编码图
                #                   image.cpu()[:images_to_save, :, :, :]-encoded_images[:images_to_save, :, :, :].cpu(),     # 残差图
                #                   epoch,
